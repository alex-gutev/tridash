...
when their identifier appears in source code. This can either be a
declaration consisting of the identifier itself or a functor node
...
functor node. A change in the value of at least one argument node results
...
'dirtied'. If a node is dirtied, all its observers are dirtied, and
likewise their observers are dirtied and so on. A node with multiple
dependencies will only recompute its value when it receives a value
change notification from each of its dirtied dependency nodes. Thus
there is no intermediate value where the node's value is recomputed
...
nodes `b` and `c` of node `+(b, c)`. A change in `a` will 'dirty' the
following nodes:
...
created on each functor node declaration. The created context has the
...
as having been triggered by a single common ancestor node. See
<<_propagation_of_changes, Propagation of Changes>> for the
...
which does not 'fail' to evaluate to a value, _see <<_failures,
Failures>>._
...
explicit contexts in source code.
...
evaluates to 'false', or by the `fail` meta-node.

Operations which expect an argument node to evaluate to a value will
...
fails to evaluate to a value, is evaluated, the entire function fails
...
evaluates to 'false', `b` is not set to the value of `a` but is set to
a failure value.
...
# numeric value greater than zero or a fail.
...
--------------------------------------------------
...
see <<_explicit_contexts, Explicit Contexts>>. If a condition node
evaluates to 'false', it is treated as though the corresponding
dependency node has failed to evaluate to a value. The context's
function then evaluates to the next dependency which does not fail to
evaluate to a value. If all condition nodes evaluate to 'false', the
node fails to evaluate to a value.
...
The failure type can of a failed value can be retrieved with the
...
in which failures are returned, there is no failure type. This will be
...
function that compares the failure type to a given value.
...
listing of some attributes an a summary of their effect:
    ...
    `macro`:: If set to true the meta-node is treated as a macro node.
    `target-node`:: Sets the name of a meta-node to use as the value
    function, in the contexts of the binding of the meta-node instance
    (as the source node) to its arguments (as the target nodes).
...
.Example
--------------------------------------------------
string-concat(person.first-name, person.last-name) -> full-name
--------------------------------------------------
...
If a subnode fails to evaluate to a value, it does not cause the
parent node to fail to evaluate to value. The parent node evaluates to
a dictionary however the dictionary entry, corresponding to the
subnode, evaluates to a failure.
