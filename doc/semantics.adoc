...
active if the condition node `c` evaluates to a 'true' value, the
value of the builtin `True` node,. If `c` evaluates to 'false', the
value of the builtin `False` node, `b` is not set to the value of `a`
but is set to a failure value.
