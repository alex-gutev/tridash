...
Evaluates to true if `x` evaluates to a value. If `x` evaluates to a
...
Returns <<node-true,`True`>> if `x` and `y` evaluate to
`True`. Returns <<node-false, `False`>> otherwise.
...
Returns <<node-true,`True`>> if either `x` or `y` evaluate to
`True`. Returns <<node-false, `False`>> if both `x` and `y` evaluate
to `False`.
...
Returns <<node-true,`True`>> if `x` evaluates to <<node-false,`False`>>.
...
clause of which the `condition` node evaluates to
<<node-true,`True`>>. The final clause may also be of the form
`value`, in which case it becomes the default value, to which the
`case` expression evaluates if the conditions of all the other clauses
evaluate to <<node-false,`False`>>.
...
The empty list is represented by the value of the node <<node-Empty,
`Empty`>>.
...
If `list` is the empty list, returns a failure of type
<<node-Empty, `Empty`>>.

If `list` is not a list returns a failure value of type
<<node-type-error, `Type-Error`>>.
...
NOTE: Does not return true if `thing` is the empty list.
...
The value of this node is a failure of type <<node-Empty,
`Empty`>>. This failure is returned when attempting to access the
...
Unlike `list`, if at least one of `xs` fails to evaluate to a value,
...
Returns a failure of type <<node-index-out-bounds,
`Index-Out-Bounds`>> if `n` is greater than the number of elements in
`list`.
...
Same as <<meta-node-foldl1,`foldl'`>> except the <<meta-node-head,
'head'>> of `list` is used as the initial first argument to the fold
function `f`.
...
returns a failure of type <<node-index-out-bounds,`Index-Out-Bounds`>>.
...
If `dict` does not have an entry for `key` a failure, of type
<<node-no-value,`No-Value`>> is returned.
...
If `condition` evaluates to a failure, a condition of `True` is
