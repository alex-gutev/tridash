...
Matches if the source node evaluates to a failure. The argument `x` is
ignored. _See <<pattern_matching>>_.
...
NOTE: This is pattern matches even if not all its nested patterns have
matched.
...
Matches the empty list. _See <<pattern_matching>>_.

NOTE: This is effectively a constant pattern.
...
[[match-fail]]
==== Meta-Node: Match-Fail() ====

(((Match-Fail, Core Module, Meta-Node)))

Returns a failure with the type `&(Match-Fail)`.

This failure type is used to indicate that a pattern failed to match.
...
with type <<match-fail,`&(Match-Fail)`>>, otherwise returns 'true'.
...
context which is activated on failures with type <<match-fail,`&(Match-Fail)`>>.
